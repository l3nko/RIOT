...
static int _send(gnrc_netdev_t *netdev, gnrc_pktsnip_t *pkt)
{
    ...
    DEBUG("%s frame header length %"PRIu8"\n", __PRETTY_FUNCTION__, (uint8_t)len);
    ...
    DEBUG("%s snip length %"PRIu8"\n", __PRETTY_FUNCTION__, (uint8_t)snip->size);

    cc2420_tx_prepare(dev); //RX_OFF + TX_FLUSH + TX_ON

    /* put header into FIFO */
    len = cc2420_tx_load(dev, mhr, len);
    DEBUG("%s load header length=%"PRIu8"\n", __PRETTY_FUNCTION__, (uint8_t)len);

    /* load packet data into FIFO */
    while (snip) {
        len = cc2420_tx_load(dev, snip->data, snip->size);
        DEBUG("%s load snip length=%"PRIu8"\n", __PRETTY_FUNCTION__, (uint8_t)len);
        snip = snip->next;
    }
